...
export const createWebHook = (values: any, extPath: string) => {
  ...
  return moduleLoader(extPath).then((modules) => {
    ...
    return sp.web.lists.getById(values.ListId).subscriptions.add(values.HookUrl, values.expirationDate, values.ClientState).then(() => {
      return {
        success: true,
        result: [],
        errorMessage: '',
