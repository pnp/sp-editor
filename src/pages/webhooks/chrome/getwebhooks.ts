...
export const getWebHooks = (extPath: string) => {
  ...
  return moduleLoader(extPath).then((modules) => {
    ...
    return sp.web.lists.expand('Subscriptions').select('Id, Title, Subscriptions').orderBy('Title', true)().then((result: any[]) => {
