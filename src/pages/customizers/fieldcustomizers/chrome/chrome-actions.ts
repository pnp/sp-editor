...
// Interface for field customizer from app catalog
...
// Get field customizers from tenant app catalog
const getFieldCustomizersFromAppCatalog = (extPath: string) => {
  type libTypes = [any, any, any]
...
// Export function to load field customizers from app catalog
export const loadFieldCustomizersFromAppCatalog = async (
  tabId: number
): Promise<IAppCatalogFieldCustomizer[]> => {
  try {
    const extPath = chrome.runtime.getURL('/')
    ...
    const result = await chrome.scripting.executeScript({
      target: { tabId },
      world: 'MAIN',
      func: getFieldCustomizersFromAppCatalog,
      args: [extPath],
    })
