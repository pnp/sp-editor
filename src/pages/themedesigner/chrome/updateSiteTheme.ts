/**
 * Updates an existing site theme using the brand center API
 */
export function updateSiteTheme(themeId: number, themeName: string, themePalette: { [key: string]: string }, isInverted: boolean) {
  ...
  return moduleLoader().then(function() {
    ...
    console.log('[SP Editor] Updating site theme:', themeName, 'id:', themeId, 'url:', webUrl);
    ...
    return getRequestDigest(webUrl).then(function(digest) {
      ...
      console.log('[SP Editor] Calling UpdateSiteTheme API...');
      
      return fetch(webUrl + '/_api/brandcenter/UpdateSiteTheme', {
        method: 'POST',
        headers: {
      ...
      .then(function(response) {
        console.log('[SP Editor] UpdateSiteTheme response status:', response.status);
        if (!response.ok) {
      ...
      .then(function(data) {
        console.log('[SP Editor] Theme updated successfully:', data);
        return {
          success: true,
          result: data,
          errorMessage: '',
    ...
    console.error('[SP Editor] updateSiteTheme error:', error);
    return {
      success: false,
      result: null,
      errorMessage: error.message || 'Failed to update site theme',
      source: 'chrome-sp-editor',
    };
  });
