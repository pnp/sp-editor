 ...
 */
export const PNPJS_CONSOLE_PROXY_SOURCE = `(function(){
  if (window.__spEditorPnpjsConsoleProxied) return;
  window.__spEditorPnpjsConsoleProxied = true;
  function fmt(a){
    try {
  ...
  function send(level, args){
    try {
      window.postMessage(JSON.stringify({
        source: 'sp-editor-pnpjs-console',
        level: level,
  ...
  ['log','info','warn','error','debug'].forEach(function(level){
    ...
    console[level] = function(){ send(level, arguments); if (orig) try { orig.apply(null, arguments); } catch(_e){} };
  });
